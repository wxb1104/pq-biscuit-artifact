...
> **Post-Quantum Biscuit: Deployability and Performance Impact of Post-Quantum Signatures on Offline-Attenuating Capability Tokens**
...
python3 experiments/netbed/analyze_net.py                # netfig1-5 from CSV
```
...
network figures and tables from the committed raw CSV, `analyze_net.py` alone
takes a few seconds.
...
| End-to-end authorization latency vs RTT (server cost amortized) | `netbed_client latency` → `net_latency.csv` | `analyze_net.py` → `netfig1`, `en1_summary.csv` |
| Server-cost share $\rho=t_{\mathrm{srv}}/L$ vs RTT | `net_latency.csv` | `analyze_net.py` → `netfig2`, `en1_fits.csv` |
| Loss inflates the tail, 0% application failures | `netbed_client latency` → `net_latency.csv` | `analyze_net.py` → `netfig3`, `en2_loss.csv` |
| Throughput shifts verification-bound → network-bound | `netbed_client throughput` → `net_throughput.csv` | `analyze_net.py` → `netfig4`, `en3_throughput.csv` |
| Constrained-link completion linear in token size | `netbed_client latency` → `net_latency.csv` | `analyze_net.py` → `netfig5`, `en4_bandwidth.csv` |
| Wire segmentation matches $\lceil S/M\rceil$ | tcpdump → `net_segments_meas.csv` | `analyze_net.py` → `en5_segments.csv` |
| Binary and resident-memory footprint | `netbed_server`, `/proc` → `net_footprint.csv` | `analyze_net.py` → `en6_footprint.csv` |
...
`*_summary.json` files. `<tag>` is `avx2` or `portable`. Network analysis
output lives in `experiments/netbed/analysis/`: `en1_summary.csv`,
`en1_fits.csv`, `en1_overhead_vs_ed.csv`, `en2_loss.csv`,
`en3_throughput.csv`, `en4_bandwidth.csv`, `en5_segments.csv`, and
...
`netbed/common.rs`), the `experiments/netbed` scripts, and `analyze_net.py` are
the network measurement code; `impl/pqbiscuit/src/bin` and
`experiments/pqbench` are the token-layer and standalone measurement code for
this paper.
