...
pub fn public_key(i: &str) -> IResult<&str, builder::PublicKey, Error> {
    alt((
        ...
        preceded(tag("mldsa44/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Mldsa44,
        }),
        preceded(tag("mldsa65/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Mldsa65,
        }),
        preceded(tag("mldsa87/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Mldsa87,
        }),
        preceded(tag("fndsa512/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Fndsa512,
        }),
        preceded(tag("fndsa1024/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Fndsa1024,
        }),
        preceded(tag("slhdsa128s/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Slhdsa128s,
        }),
        preceded(tag("slhdsa128f/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Slhdsa128f,
        }),
        preceded(tag("slhdsa256s/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Slhdsa256s,
        }),
        preceded(tag("slhdsa256f/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::Slhdsa256f,
        }),
        preceded(tag("hybrid-ed-mldsa44/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::HybridEdMldsa44,
        }),
        preceded(tag("hybrid-ed-fndsa512/"), parse_hex).map(|key| PublicKey {
            key,
            algorithm: builder::Algorithm::HybridEdFndsa512,
        }),
