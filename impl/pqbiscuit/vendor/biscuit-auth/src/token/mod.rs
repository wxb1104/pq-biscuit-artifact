...
impl Biscuit {
    ...
    pub(crate) fn new_with_rng<T: RngCore + CryptoRng>(
        ...
        // Post-quantum migration: for a post-quantum root key, pick a
        // same-algorithm next key so the authority block starts a PQ-only chain.
        // Classical (Ed25519/secp256r1) roots keep the historical Ed25519
        // default to preserve backward compatibility and interoperation.
        let next_algorithm = match root {
            KeyPair::Mldsa44(_) => builder::Algorithm::Mldsa44,
            KeyPair::Mldsa65(_) => builder::Algorithm::Mldsa65,
            KeyPair::Mldsa87(_) => builder::Algorithm::Mldsa87,
            KeyPair::Fndsa512(_) => builder::Algorithm::Fndsa512,
            KeyPair::Fndsa1024(_) => builder::Algorithm::Fndsa1024,
            KeyPair::Slhdsa128s(_) => builder::Algorithm::Slhdsa128s,
            KeyPair::Slhdsa128f(_) => builder::Algorithm::Slhdsa128f,
            KeyPair::Slhdsa256s(_) => builder::Algorithm::Slhdsa256s,
            KeyPair::Slhdsa256f(_) => builder::Algorithm::Slhdsa256f,
            KeyPair::HybridEdMldsa44(_) => builder::Algorithm::HybridEdMldsa44,
            KeyPair::HybridEdFndsa512(_) => builder::Algorithm::HybridEdFndsa512,
            _ => builder::Algorithm::Ed25519,
        };
        Self::new_with_key_pair(
            root_key_id,
            root,
            &KeyPair::new_with_rng(next_algorithm, rng),
            symbols,
            authority,
        )
    }
